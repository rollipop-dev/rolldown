...
export interface RollipopReactNativeSwcConfig {
  /**
   * SWC `.wasm` plugins to load. An optional third entry filters only this
   * plugin's execution, not the rest of the React Native transform pipeline.
   * Filters support `code` predicates and boolean combinators, evaluated against
   * the original transform input, not code generated by preceding WASM plugins.
   */
  plugins?: [
    path: string,
    config: Record<string, unknown>,
    options?: { filter?: TopLevelFilterExpression[] },
  ][];
  /**
...
function lowerSwc(swc: RollipopReactNativeSwcConfig | undefined) {
  ...
  return {
    plugins: swc.plugins?.map(([path, pluginConfig, options]) => ({
      path,
