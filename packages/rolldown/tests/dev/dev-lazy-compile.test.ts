...
import type { InputOptions, OutputOptions, Plugin } from '@rollipop/rolldown';
import type { DevEngine, DevOptions } from '@rollipop/rolldown/experimental';
import { dev as _dev } from '@rollipop/rolldown/experimental';
...
test(
  ...
  async ({ onTestFinished }) => {
    ...
    const { DevRuntime } = await import(
      import.meta.resolve('@rollipop/rolldown/experimental/runtime')
    );
